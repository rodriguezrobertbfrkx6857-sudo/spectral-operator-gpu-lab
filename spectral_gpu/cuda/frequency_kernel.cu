...
torch::Tensor frequency_mul_forward(torch::Tensor input, torch::Tensor weight,
    ...
    TORCH_CHECK(input.is_cuda() && weight.is_cuda(), "frequency_mul_forward requires CUDA tensors");
    ...
    TORCH_CHECK(input.size(1) == weight.size(0) && weight.size(2) == modes1 && weight.size(3) == modes2,
                "incompatible frequency dimensions");
    input = input.contiguous();
    weight = weight.contiguous();
    auto output = torch::zeros({input.size(0), weight.size(1), input.size(2), input.size(3)}, input.options());
    ...
    frequency_mul_kernel<<<blocks, threads, 0, stream>>>(
        ...
        input.size(0), input.size(1), weight.size(1), input.size(2), input.size(3), modes1, modes2);
