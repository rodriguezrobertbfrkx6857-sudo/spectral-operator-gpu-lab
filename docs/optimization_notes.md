...
3. Custom CUDA path: one thread computes one output frequency/channel and accumulates complex products in registers on the active PyTorch CUDA stream.
4. Optional fused path: a CUDA kernel combines frequency selection and complex multiplication into one output-spectrum write. It selects `weight_positive` for the top retained band and `weight_negative` for the bottom retained band. It is opt-in with `SPECTRAL_GPU_ENABLE_FUSED=1` and is never assumed to be faster without a benchmark.
