...
- The custom CUDA operator is an inference path; no custom backward implementation is provided. Use the reference layer for training or add and validate an autograd implementation before doing so.
- FFT execution remains delegated to `torch.fft`; this project is not an FFT implementation.
- The fused kernel is opt-in and requires independent correctness and performance validation on the target GPU. It accepts separate positive- and negative-frequency weight tensors.
